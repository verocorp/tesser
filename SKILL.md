...
1. **Spawn the background subagent, then answer immediately — from what you already know, or from a fast README read when you don't (contract:readme-first).** Your *first* action launches a subagent (`run_in_background: true`) whose whole prompt is: *read `subagent-brief.md` and ground <the question> for <the repo>, silently; return the grounded summary.* Then answer, with the source by which you know:
   ...
   - **You don't recognize it** — the founding case, an unfamiliar dependency — → do **one** fast, SHA-pinned README/description fetch (`gh api` / raw at the resolved HEAD, never a clone) and answer from that, conveyed plainly as the project's own documentation ("going by its README, …"), never as verified behavior. That is real grounding the dev can use in seconds, not a "hang on, I don't know this" holding line — a holding line is the speed failure: it doesn't count as an answer, so the dev ends up waiting for the clone anyway. The background source read then confirms or sharpens it.

   The README fetch is the **only** foreground step this licenses — it *is* the answer's source. `git pull`, the log, the map check, and the clone stay the background subagent's job, never a foreground step you run first. No "I'll use the tesser skill", no "looking into it", no progress lines. The grounding **never blocks the answer**: under 30 seconds, answer first, close on the dev's next job.
