...
state variables first makes a change *local* — it touches one variable's column, not
the whole graph — and the cross-cutting invariants explicit and checkable instead of
re-derived by hand.

**How it was derived:** a v1 set (a static/derived three-layer partition) was torn down
by three independent red-teams (the author + two fresh-context adversarial agents, one
MECE-lens, one FSM/spec-lens). They converged: the static/derived cut ran through the
middle of one homogeneous class, three "static" members mutate mid-run, and the
layer boundary bisected a feedback loop. v2 re-cuts by **provenance-of-value**.

**Membership test** (every variable lands in exactly one):

> **A** given by the dev/world this turn · **B** produced by *running a node*,
> re-evaluated as you go · **C** loaded from `~/.tesser/` at run start, written at
> run end · **D** a cursor threaded *between* B-acts · **E** a discrete event that arrives

The subtlest line is **B vs D**: B is "what a node just returned," D is "the cursor I
carry between nodes, which B-acts update."

---

## A · Given per turn (re-given each turn; can change on the next)
...
| **Reference, as supplied** | the raw token/URL + did the dev give a URL? | The *subject* provenance (dev-named vs not) — distinct from the URL provenance in D. |
...
## B · Derived by acting (you don't know it until you run something; re-evaluated as evidence arrives)

The honest home for everything learned by doing. v1's "static" identity/reachability
and "loop" confidence/sufficiency collapse here with the three axes v1 missed — all the
same kind: a node's return value.

| Variable | Values | Notes |
|---|---|---|
| **Model-knows-the-dep?** | recognizes / doesn't | Drives beat-1 source (knowledge answer vs foreground grounding) — the 25s-vs-163s difference (`SKILL.md:46-47`, `b40f75e1`). |
| **`dependency_kind`** | clonable-cli / clonable-library / sdk-of-hosted-service / hosted-closed / heavy-infra | Preflight classification (`digest-schema.yaml:64`). The verification **ceiling**, and it governs whether the uniform-clone shape applies (clonable kinds) or a docs-only branch does (non-clonable). |
| **Identity-resolution outcome** | unresolved / resolved+pinned / **404-ambiguous (gone-or-private)** | *One* observation (the `ls-remote` act). "Reachability" is **not** a separate axis — inaccessible *is* the 404 value, refined only by the S8 retry. |
| **Reachability refinement** | public-ok / ambient-creds-ok / inaccessible | A *sub-outcome* of the same act + S8 ambient-creds retry; only meaningful after a 404. |
| **Subject-identity confidence** | sure / unsure | Re-judged at interpret → search-candidates → README. **Co-evaluated with sufficiency** at the README read — a mismatch is *also* insufficient; not orthogonal, stated honestly. |
| **Sufficiency** | enough / climb one rung | Re-judged per rung. This judgment and the rung-path output are **one feedback loop** — the judgment is here (B), its cumulative consequence is an output. |
| **Action outcome + proxy-trust** | success / nonzero-exit / timeout / errored — **and is the success signal trustworthy?** | The D48 axis: exit 0 / HTTP 200 / enqueued-id ≠ did-the-thing. |
| **Observability** | agent-observable / dev-must-observe | For does-it-work/make-it-work: can tesser watch the ground-truth signal itself, or must it instruct the dev? Independent of altitude. Determines whether the answer ships the full delegation verification-contract. |

## C · Persisted across invocations (`~/.tesser/`; read at start, written at end — the channel that couples outputs back to inputs)

The category v1 missed entirely; ignoring it is *why* v1's locality claim failed.

| Variable | Values | Notes |
|---|---|---|
| **Local supply** | clone (+ materialization level) / map / build present | Floors the reuse short-circuit. `map ⟹ clone` (see invariant 5). With the uniform sparse-clone shape, "clone" carries a materialization level (sparse-README → wider → full tree → history). |
| **Prior `truth_grade`** | docs / inspect / run | A stored map's grade short-circuits *this* run's build path (`subagent-brief.md:64`). Last run's output → this run's input. |
| **Drift state** | reused-unchecked / confirmed-current / drift-found | Set on a warm serve, resolved async. |

## D · Mutable control-state carried within a run (the cursor B-acts update; later decisions read it)

| Variable | Values | Notes |
|---|---|---|
| **Working reference + URL-provenance** | dev-given / recalled / search-derived | **Flips** when a recalled URL 404s → search picks a new one (`grounding-fsm.html:76`). |
| **Ladder cursor** | current materialization/rung | The live position; distinct from the full path run (an output). |

## E · Input events mid-flow (not standing state — discrete arrivals)

| Event | Branches | Notes |
|---|---|---|
| **Dev confirmation answer** | clarifies / can't-say / gives-URL | The CSUBJ/ASKURL/CMATCH nodes. A dev supplying a URL mid-flow is an *event*, not a dimension value. |
| **Build-completion notification** | exit 0 / nonzero | System event (not dev input) — drives the beat-2 supersede. |

---

## Derived outputs (computed from A–E; genuinely not inputs)
...
2. Reuse-first — check category C before any network.
...
## What survived all three red-teams unchanged

The **cold-path control flow** — the ladder + cold convergence on `ls-remote`. v1
over-generalized this one good thing into a universal static-fact model the warm path,
the failures, and the dev-confirmations all contradicted.
...
coverage. **No grade-enum change** — it folds into category-B obligation-coverage and the
...
- **The B-vs-D seam** — the membership line between "a node's return value" (B) and "the
  carried cursor" (D) is the subtlest; worth a pass to confirm every variable sits on the
  right side.
- **Non-clonable-kind branch** — the docs-only grounding path for `hosted-closed`/`sdk`/
  `heavy-infra` is named but not detailed.
