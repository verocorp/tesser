...
export const KNOWN_FAILURE_MODES: FailureMode[] = [
  ...
  { mode: "Half-contract acceptance: passing a does-it-work answer that says how to confirm success but omits the signal that will FALSELY read as failure (or success).", correction: "For a result claim (principle 11), a verification instruction is complete only if it BOTH names the ground-truth signal AND PROACTIVELY warns off the misleading proxy. A positive-only signal with no distrust warning is a HALF-CONTRACT — PARTIAL at best, never PASS. Watch the SOFT variant (the B3i shape): an answer that leads with the right ground-truth check but only gates or back-references the lying proxy as an afterthought — never an explicit 'don't trust X, it reads PENDING/200 even on success' — is still PARTIAL, because the dev reaches for the tempting signal whether or not it was named. Do NOT apply this to an overview that asserts no result; there it is N/A." },
];
