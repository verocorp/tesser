...
- `log.jsonl` — one line each time you ask it something, recording what it did: the repo and exact commit it looked at, your question, what happened, and the command and exit code if a step failed. If a run stops partway, its line is just left unfinished. The full set of fields is documented in `log-schema.yaml`.
